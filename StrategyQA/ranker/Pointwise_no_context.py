...
class PointwiseRankDataset(Dataset):
    ...
    def _create_pairs(self):
        ...
        for idx, row in self.data.iterrows():
            ...
            for query, score in rewritten_questions:
                normalized_score = score / 1.0  # 假设分数最大为10，进行标准化
                pairs.append((query, original_question, normalized_score))
        return pairs
    ...
    def __getitem__(self, idx):
        query, original_question, label = self.pairs[idx]

        # 计算剩余的长度以保持整体不超长
        remaining_length = self.max_length - 3  # 3为[CLS], [SEP], [SEP]的长度
        len_query = len(self.tokenizer.tokenize(query))
        len_original = remaining_length - len_query
        original_tokens = self.tokenizer.tokenize(original_question)[:len_original]
        original_question = self.tokenizer.convert_tokens_to_string(original_tokens)
...
loss_curve_path = os.path.join(model_dir, 'pointwise_loss_curve_no_context.png')
plt.savefig(loss_curve_path)  # 保存损失曲线图
plt.show()

model_save_path = os.path.join(model_dir, "pointwise_ranker_no_context.pth")
torch.save({
