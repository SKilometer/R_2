...
"""
    生成（question，context, Sorted（query，score））的文件
    读取hotpotqa_test_qr.csv文件，依次读取question，
    每个重写问题与向量库进行匹配检索k个相关的chunks
    将（原始问题，chunk）送入evaluate_relevance进行0-1打分
    将k个chunks得分相加得到一个重写问题的得分
    最后将（question，context, Sorted（query，score））进行保存 
    生成hotpotqa_test_qr_score.csv文件
...
def get_rank(question, re_queries, retriever):
    ...
    for requery in re_queries:
        ...
        for doc in retrieved_docs:
            score = evaluate_relevance(question, doc.page_content)
            total_score += score
...
def process_csv(input_csv, output_csv):
    ...
    with open(input_csv, 'r', encoding='utf-8-sig') as csv_in, open(output_csv, 'w', newline='', encoding='utf-8-sig') as csv_out:
        ...
        writer = csv.DictWriter(csv_out, fieldnames=reader.fieldnames + ["Relevance Context", "Rewritten Queries with Scores"])
        writer.writeheader()
        for row in tqdm(reader):
            ...
            if question:
                ...
                row.update({
                    "Relevance Context": context_str,
                    "Rewritten Queries with Scores": queries_scores
                })
                writer.writerow(row)


medmaqa_train_filename = './dataset/hotpotqa_test_qr.csv'
csv_filename = './dataset/hotpotqa_test_qr_score.csv'
