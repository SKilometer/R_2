...
ollama_eval_llm = Ollama(model='qwen2.5:1.5b')
...
rag_eval_chain = rag_prompt | model | StrOutputParser()
...
for index, row in tqdm(df.iterrows(), total=len(df)):
    ...
    if relevant_context is not None:
        result = rag_eval_chain.invoke({"question": question, "context": relevant_context}).replace('"', '').replace('.', '').replace("'", "")
    else:
        count += 1
        result = rag_chain.invoke({"question": question, "context": retrieved_docs}).replace('"', '').replace('.', '').replace("'", "")
