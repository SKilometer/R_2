...
input_csv_path = './dataset/boolq.csv'  # 输入 CSV 文件路径
chroma_persist_dir = './chromadb/chroma_boolq'  # Chroma 持久化目录
output_chunks_csv = './dataset/passage_chunks.csv'
...
def process_exp_column(csv_path):
    ...
    if 'passage' not in df.columns:
        raise ValueError("输入的 CSV 文件中不存在 'passage' 列！")

    documents = []
    chunk_data = []
    print("正在分割 'passage' 列中的文本...")
